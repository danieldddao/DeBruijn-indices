...
public class TreePrinter {
    ...
    public static <T extends Comparable<?>> void printNode(Tree<T> root) {
        int maxLevel = TreePrinter.maxLevel(root);

        printNodeInternal(Collections.singletonList(root), 1, maxLevel);
    }

    private static <T extends Comparable<?>> void printNodeInternal(List<Tree<T>> nodes, int level, int maxLevel) {
        ...
        List<Tree<T>> newNodes = new ArrayList<Tree<T>>();
    ...
    private static <T extends Comparable<?>> int maxLevel(Tree<T> node) {
        if (node == null)
            return 0;
